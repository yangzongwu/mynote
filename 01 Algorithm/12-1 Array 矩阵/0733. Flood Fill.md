...
class Solution:
 ...
            return image
        
        self.refillImage(image,sr,sc,image[sr][sc],newColor,)
        return image
    
    
    def refillImage(self,image,row,col,target,newColor):
        image[row][col]=newColor
        if row-1>=0 and image[row-1][col]==target:
            self.refillImage(image,row-1,col,target,newColor)
        if col-1>=0 and image[row][col-1]==target:
            self.refillImage(image,row,col-1,target,newColor)
